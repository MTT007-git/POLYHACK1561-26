...
def quiz(message):
    ...
    if not daily_quiz or daily_quiz.get("date") != today:
        bot.send_message(message.chat.id, f"⏳ Квиз еще не готов. Квизы генерируются в {GEN_TIME} каждый день.")
        return
...
def send_question(chat_id):
    ...
    if q_type == "multiple_choice":
        ...
        items_text = "\n".join(q["items"])
        for item in q["items"]:
            if item[0] in ['1', '2', '3', '4', '5']:
                markup.add(types.InlineKeyboardButton(item, callback_data=f"ans_{item[0]}"))
        msg = bot.send_message(chat_id, f"❓ Вопрос {quiz['current']+1}/{len(quiz['questions'])}:\n\n{q['q']}\n{items_text}\n\nВведите ответ в формате: 1-A, 2-B, 3-C")
        bot.register_next_step_handler(msg, lambda m: process_matching_answer(m, chat_id))
        quiz["last_msg_id"] = msg.message_id
        return
    
    elif q_type == "sequence":
        items_text = "\n".join(q["items"])
        msg = bot.send_message(chat_id, f"❓ Вопрос {quiz['current']+1}/{len(quiz['questions'])}:\n\n{q['q']}\n{items_text}\n\nВведите последовательность букв через запятую (например: A, B, C)")
        bot.register_next_step_handler(msg, lambda m: process_sequence_answer(m, chat_id))
        quiz["last_msg_id"] = msg.message_id
        return
...
def process_open_answer(message, chat_id):
    ...
    if is_correct:
        quiz["score"] += 1
        quiz["bonus_points"] = quiz.get("bonus_points", 0) + 1
        msg = bot.send_message(chat_id, "✅ Правильно! +1 очко")
    else:
...
def finish_quiz(chat_id, user):
    ...
    bonus_points = quiz.get("bonus_points", 0)
    result_text = f"🎊 Квиз завершён!\nВаш результат: {quiz['score']}/{len(quiz['questions'])}\n\n"
    ...
    for i, q in enumerate(quiz["questions"]):
        ...
        if q_type == "open_answer":
            # Для открытых ответов используем сохраненный результат проверки
            if isinstance(user_ans, dict):
                status = "✅" if user_ans.get("is_correct") else "❌"
                result_text += f"{status} {i+1}. {q['q']}\n"
                result_text += f"Ваш ответ: {user_ans['text']}\n"
                result_text += f"Правильный: {correct_ans}\n\n"
            else:
                result_text += f"❓ {i+1}. {q['q']}\n"
                result_text += f"Ваш ответ: {user_ans}\n"
                result_text += f"Правильный: {correct_ans}\n\n"
        elif q_type == "multiple_choice":
            status = "✅" if user_ans == correct_ans or user_ans == correct_ans[0] else "❌"
            result_text += f"{status} {i+1}. {q['q']}\n"
            result_text += f"Ваш ответ: {user_ans}\n"
            result_text += f"Правильный: {correct_ans}\n\n"
        else:
            status = "✅" if str(user_ans).replace(" ", "").lower() == str(correct_ans).replace(" ", "").lower() else "❌"
            result_text += f"{status} {i+1}. {q['q']}\n"
            result_text += f"Ваш ответ: {user_ans}\n"
            result_text += f"Правильный: {correct_ans}\n\n"
    ...
    if quiz["score"] == len(quiz["questions"]):
        # Все правильно - даем 3 очка вместо 1
        data["users"][user_id]["points"] += 3
        data["users"][user_id]["perfect_quizzes"] = data["users"][user_id].get("perfect_quizzes", 0) + 1
        save_data(data)
        result_text += f"🎁 Идеально! Вы получили 3 очка! Всего очков: {data['users'][user_id]['points']}"
    else:
        # Начисляем бонусные очки за сложные вопросы
        data["users"][user_id]["points"] += bonus_points
        save_data(data)
        if bonus_points > 0:
            result_text += f"💰 Вы получили {bonus_points} очко(в) за сложные вопросы! Всего очков: {data['users'][user_id]['points']}"
